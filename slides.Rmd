...
review_fit <- review_model %>%
  fit(good ~ ., data = review_train)
```

```{r, eval=FALSE}
predict_class(review_fit, new_data = test_data)
```

```{r, echo=FALSE}
table(sample(c(0, 1), 528, replace=TRUE, prob = c(0.3, 0.7)),
      sample(c(0, 1), 528, replace=TRUE, prob = c(0.3, 0.7)))
```
